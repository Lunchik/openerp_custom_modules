...
class revenue_by_cc_client(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("""
            ...
            group by partner_id, parent_id, short_name
            order by partner_id;
    ...
    def init(self, cr):
        ...
        cr.execute("""
           ...
           p.ref as short_name,
            ...
            from  account_invoice i, res_partner p where i.type='out_invoice' and state in ('open', 'paid') and i.partner_id=p.id
...
class salesRevenueReport(object):
    ...
    def generate(self):
        ...
        if filter == 'avg_cou':
            sheet_name = 'Revenue by Avgjob by Country'
            self.sheet = self.book.add_sheet(sheet_name)
            self.generate_avg('Sale By AVGJob/Country Report', 'sale_by_avg_cou_report','sale_by_avg_cou_report2', 'sale.by.avg.cou.report','sale.by.avg.cou.report2')
        if filter == 'avg_po':
            sheet_name = 'Revenue by Avgjob by Client'
            self.sheet = self.book.add_sheet(sheet_name)
            self.generate_avg('Sale By AVGJob/Client Report', 'sale_by_avg_pa_report','sale_by_avg_pa_report2', 'sale.by.avg.pa.report','sale.by.avg.pa.report2')
        if filter == 'avg_opr':
            sheet_name = 'Revenue by Avgjob by operator'
            self.sheet = self.book.add_sheet(sheet_name)
            self.generate_avg('Sale By AVGJob/opr Report', 'sale_by_avg_opr_report','sale_by_avg_opr_report2', 'sale.by.avg.opr.report','sale.by.avg.opr.report2')
        if filter == 'avg_de':
            sheet_name = 'Revenue by Avgjob by operator'
            self.sheet = self.book.add_sheet(sheet_name)
            self.generate_by_avgde()
        if filter == 'service3':
            sheet_name = 'Revenue by service by category '
            self.sheet = self.book.add_sheet(sheet_name)
            self.generate_common('Revenue by Service by category', 'sale_by_service_cat2_report', 'sale.by.service.cat2.report')
    ...
    def generate_common(self, title_name, report_name, report_dot_name):
        ...
        for c in cat_ids:
            ...
            for rec in obj.browse(self.cr, self.uid, ids, context=self.context):
                if report_name is 'sale_by_service_cat2_report':
                    cat = rec.defcol_two_id
                else:
...
######AVG generate
    ...
    def generate_avg(self, title_name, report_name,report_name2, report_dot_name,report_dot_name2):
        ...
        for c in cat_ids:
            ...
            cat = ''
            for rec in obj.browse(self.cr, self.uid, ids, context=self.context):
                if report_name is 'sale_by_avg_de_report':
                    cat = rec.defcol_two_id
                else:
        ...
        obj2 = self.pool.get(report_dot_name2)
        idss= bro.search(self.cr,self.uid,[])
        obj2.init11(self.cr, self.uid, idss)
        ...
        for f in cat_ids2:
            ids = obj2.search(self.cr, self.uid, [('defcol_two_id', '=', f)], context=self.context)
            totaly = {
                1: 0.0,
                2: 0.0,
                3: 0.0,
            ...
            i += 1  
    def generate_by_avgde(self):
           
        self.sheet.write(1, 1, 'Sale By AVGJob/Detailed Report', self.header_style)
        self.sheet.write(1, 2, 'Jan', self.header_style)
        self.sheet.write(1, 3, 'Feb', self.header_style)
        self.sheet.write(1, 4, 'Mar', self.header_style)
        self.sheet.write(1, 5, 'Q1', self.header_style)
        ...
        self.cr.execute("""
            SELECT DISTINCT defcol_two_id  FROM sale_by_avg_de_report
            """)
        cat_ids = [c[0] for c in self.cr.fetchall()]
        ...
        i = 2
        for c in cat_ids:
            obj = self.pool.get('sale.by.avg.de.report')
            ids = obj.search(self.cr, self.uid, [('defcol_two_id', '=', c)], context=self.context)
            totaly = {
                1: 0.0,
                2: 0.0,
        ...
        obj2 = self.pool.get('sale.by.avg.de.report2')
        idss= bro.search(self.cr,self.uid,[])
        ...
        i += 1
        self.cr.execute("""
                SELECT DISTINCT id FROM sale_by_avg_de_report2 where defcol_two_id ='other'
                """)
        cat_ids2 = [f[0] for f in self.cr.fetchall()]
        ...
        for rec in obj2.browse(self.cr, self.uid, cat_ids2, context=self.context):
                cat = 'OTHERS'
                self.sheet.write(i, 1, rec.defcol_one_id.name)
                self.sheet.write(i, 2, rec.jan, self.num_style1)
                totaly[1] += rec.jan
