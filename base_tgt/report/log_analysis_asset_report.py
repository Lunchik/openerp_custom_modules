...
class LogAnalysisReport(object):
    ...
    def filter(self):
        ...
        sql = """
            SELECT asset_category FROM res_company
            WHERE id in %s
        """
        self.cr.execute(sql, (tuple(company_ids),))
        categories = [ catid[0] for catid in self.cr.fetchall() ]
        sql = """
             select to_char(
        ...
        asset_ids = asset_obj.search(self.cr, self.uid, [('category_id', 'in', categories), ('company_id', 'in', company_ids)], context=self.context)
        
        res = {}
        for rid in asset_ids:
            groups = rgroups.copy()
            emp = asset_obj.browse(self.cr, self.uid, rid, context=self.context)
            sql = """
            ...
            (select si.invoice_id from sale_order_invoice_rel si where si.order_id=s.id) limit 1)
                    ...
                    ,'mm(Month) yyyy') as mon
                 from sale_asset_rel k left join sale_order s on s.id=k.sale_id
        ...
        sql = """
            ...
            from sale_order k
            where k.id not in (select sale_id from sale_asset_rel)
            and 
            (select i.date_invoice from account_invoice i where i.id in 
            (select si.invoice_id from sale_order_invoice_rel si where si.order_id=k.id) limit 1)
            <=%s and 
    ...
    def generate(self):
        ...
        for p in res:
            ...
            break
        self.sheet.write(1, 3, 'Asset Name', self.style)
        ...
        for emp_id, data in res.iteritems():
            u = data.keys()
            u.sort()
            emp = asset_obj.browse(self.cr, self.uid, emp_id, context=self.context)
            opop = 0

            for m in u:
                opop += data[m][0] + data[m][1]
            if not opop:
                continue

            self.sheet.write(i, 3, u'{} / {}'.format(emp.name, emp.code))
            self.sheet.write(i, 2, u'{}'.format(emp.company_id.name))
            key = 3
            for m in u:
